...
class _SignupOtpVerificationState extends State<SignupOtpVerification> {
  ...
  Future<void> resendotp() async {
    try {
      ...
      final url = Uri.parse('${ApiConstant.baseUrl}/auth/verify/resend');
  ...
  otpVerification() async {
    try {
      ...
      final url = Uri.parse('${ApiConstant.baseUrl}/auth/verify');
      final headers = {'Content-Type': 'application/json'};
      final body = jsonEncode({
        "email": widget.email,
        "otp": storedOTP, // Use the stored OTP
      });
      ...
      if (response.statusCode == 200) {
        showAppToast("OTP verification successful");
